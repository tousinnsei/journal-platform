...
import { publicTrademarkInclude } from "@/lib/public-resource";
...
export default async function PublicTrademarksPage({
  searchParams,
}: {
  ...
  const pageHref = (p: number) => {
    ...
    return `/trademarks?${params.toString()}`;
  };

  return (
    <div className="space-y-8">
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold text-white tracking-tight">商标服务</h1>
        <p className="text-slate-400 text-sm">已发布的国内外商标注册、变更与续展等代办服务。</p>
      </div>

      <form method="get" action="/trademarks" className="flex items-center gap-2 max-w-md mx-auto">
          ...
          {list.map((t) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 text-xs">
                ...
                <span className="text-slate-400 line-clamp-1">{t.description || "暂无简介"}</span>
              </div>
              <div className="pt-2 border-t border-slate-800/80 flex items-center justify-between">
                <span className="text-xs text-slate-500">{t.createdBy?.name || "官方资源"}</span>
                {t.minSalePrice != null ? (
                  <span className="text-sm font-bold text-emerald-400">
                    {formatPrice(t.minSalePrice, t.currency)}{" "}
