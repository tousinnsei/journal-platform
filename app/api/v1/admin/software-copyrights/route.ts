...
import { softwareCopyrightSchema, firstErrorMessage } from "@/lib/validators";
...
export async function GET(request: Request) {
  ...
  const where: Prisma.SoftwareCopyrightWhereInput = {};
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const created = await db.softwareCopyright.create({
      data: {
        name: data.name,
    ...
    await logAuditAction(user.id, "CREATE_SOFTWARE_COPYRIGHT", "software_copyrights", created.id, {
      name: created.name,
      type: created.type,
    ...
    return NextResponse.json({ code: 200, message: "软著创建成功", data: created });
  } catch (error: unknown) {
    console.error("Create software copyright error:", error);
