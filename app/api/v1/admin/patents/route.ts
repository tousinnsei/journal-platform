...
import { patentSchema, firstErrorMessage } from "@/lib/validators";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await logAuditAction(user.id, "CREATE_PATENT", "patents", created.id, {
      name: created.name,
      type: created.type,
    ...
    return NextResponse.json({ code: 200, message: "专利创建成功", data: created });
  } catch (error: unknown) {
