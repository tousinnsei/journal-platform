...
export async function PUT(
  ...
  const existing = await db.trademarkSource.findUnique({ where: { id: sourceId } });
  ...
  try {
    ...
    const source = await db.trademarkSource.update({
      where: { id: sourceId },
      data: {
    ...
    const updated = await recomputeTrademarkPrices(id);

    await logAuditAction(user.id, "SOURCE_UPDATED", "trademark_services", id, {
      sourceId: source.id,
    ...
    return NextResponse.json({ code: 200, message: "资源渠道已更新", data: { source, trademark: updated } });
  } catch (error: unknown) {
    console.error("Update trademark source error:", error);
...
export async function DELETE(
  ...
  await db.trademarkSource.delete({ where: { id: sourceId } });
  const updated = await recomputeTrademarkPrices(id);

  await logAuditAction(user.id, "SOURCE_DELETED", "trademark_services", id, {
    sourceId: existing.id,
    sourceName: existing.sourceName,
  });

  return NextResponse.json({ code: 200, message: "资源渠道已删除", data: { trademark: updated } });
}
