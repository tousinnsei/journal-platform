...
export default function EditPatentPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/v1/admin/patents/${id}`)
      .then((r) => r.json())
      .then((data) => {
        if (cancelled) return;
        const p = (data as { data?: PatentDetail }).data;
        if (p) {
          setName(p.name || "");
          setDescription(p.description || "");
          setNote(p.note || "");
          setType(p.type || "INVENTION");
          setStatus(p.status || "DRAFT");
          setOwnerId(p.createdById || null);
          setSources(
            (p.sources || []).map((s) => ({
              id: s.id,
  ...
  const handleSourceSubmit = async (row: SourceRow) => {
    ...
    try {
      ...
      const res = await fetch(
        row.isNew ? `/api/v1/admin/patents/${id}/sources` : `/api/v1/admin/patents/${id}/sources/${row.id}`,
  ...
  const handleSourceDelete = async (row: SourceRow) => {
    ...
    setSourceSaving(true);
    try {
      const res = await fetch(`/api/v1/admin/patents/${id}/sources/${row.id}`, { method: "DELETE" });
      const data = await res.json();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "更新专利失败");
      setSuccessMsg("专利信息更新成功！");
      setTimeout(() => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <h1 className="text-xl font-bold text-white">编辑专利详情</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-between bg-slate-900/60 border border-slate-800 rounded-2xl p-6">
          <span className="text-xs text-slate-500">保存专利基本信息。</span>
          <button
            type="submit"
            disabled={submitting}
          ...
          >
            ...
            <span>{submitting ? "正在保存..." : "保存专利更新"}</span>
          </button>
        </div>
      </form>
