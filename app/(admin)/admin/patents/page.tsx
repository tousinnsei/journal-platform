...
interface AdminPatentItem {
  id: string;
  name: string;
  type: "INVENTION" | "UTILITY_MODEL" | "DESIGN";
...
export default function AdminPatentsListPage() {
  const { user } = useAuth();
  const [items, setItems] = useState<AdminPatentItem[]>([]);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        p.set("limit", "100");
        const res = await fetch(`/api/v1/admin/patents?${p.toString()}`);
        const data = await res.json();
        if (cancelled) return;
        if (!res.ok) throw new Error(data.message || "获取专利列表失败");
        setItems(data.data || []);
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`确定要物理删除专利《${name}》吗？此操作不可撤销！`)) return;
    try {
      const res = await fetch(`/api/v1/admin/patents/${id}`, { method: "DELETE" });
      const data = await res.json();
  ...
  const handleAudit = async (id: string, name: string, approve: boolean) => {
    if (!confirm(`确定要${approve ? "审核通过并发布" : "驳回"}专利《${name}》吗？`)) return;
    try {
      const res = await fetch(`/api/v1/admin/patents/${id}`, {
        method: "PATCH",
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            管理专利资源及其内部渠道价格。渠道成本价格永不对外展示。
          </p>
        </div>
        <Link
          href="/admin/patents/create"
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-4">
        ...
        <div className="mt-2 text-[11px] text-slate-500">共 {total} 条专利记录</div>
      </div>
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-500 text-sm">暂无匹配的专利记录</div>
        ) : (
                ...
                {items.map((p) => (
                  <tr key={p.id} className="hover:bg-slate-800/30 transition-colors">
                    <td className="px-4 py-4">
                      <div className="font-semibold text-white max-w-[280px] truncate">{p.name}</div>
                      <div className="text-[10px] text-slate-500 mt-0.5">
                        {p.sources.length > 0 ? `${p.sources.length} 个资源渠道` : "暂无资源渠道"}
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold border bg-slate-950 text-slate-300 border-slate-800">
                      ...
                      {p.minSalePrice != null ? (
                        <div className="space-y-0.5">
                          <div className="text-emerald-400">
                            售价 <span className="text-emerald-300 font-medium">{formatPrice(p.minSalePrice, p.currency)}</span>
                          </div>
                          <div className="text-slate-400">
                            成本{" "}
                            {p.minCostPrice != null ? (
                              <span className="text-slate-200 font-medium">{formatPrice(p.minCostPrice, p.currency)}</span>
                            ) : (
                    ...
                    <td className="px-4 py-4 text-right space-x-2 whitespace-nowrap">
                      {user?.role === "SUPER_ADMIN" && p.status === "PENDING" && (
                        <>
                          <button
                            onClick={() => handleAudit(p.id, p.name, true)}
                          ...
                          <button
                            onClick={() => handleAudit(p.id, p.name, false)}
                      ...
                      {(user?.role === "SUPER_ADMIN" || p.createdById === user?.id) && (
                        <Link
                          href={`/admin/patents/${p.id}/edit`}
