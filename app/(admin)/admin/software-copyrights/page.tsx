...
export default function AdminCopyrightsListPage() {
  const { user } = useAuth();
  const [items, setItems] = useState<AdminCopyrightItem[]>([]);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        p.set("limit", "100");
        const res = await fetch(`/api/v1/admin/software-copyrights?${p.toString()}`);
        const data = await res.json();
        if (cancelled) return;
        if (!res.ok) throw new Error(data.message || "获取软著列表失败");
        setItems(data.data || []);
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`确定要物理删除软著《${name}》吗？此操作不可撤销！`)) return;
    try {
      const res = await fetch(`/api/v1/admin/software-copyrights/${id}`, { method: "DELETE" });
      const data = await res.json();
  ...
  const handleAudit = async (id: string, name: string, approve: boolean) => {
    if (!confirm(`确定要${approve ? "审核通过并发布" : "驳回"}软著《${name}》吗？`)) return;
    try {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            管理软件著作权资源及其内部渠道价格。渠道成本价格永不对外展示。
          </p>
        </div>
        <Link
          href="/admin/software-copyrights/create"
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-4">
        <div className="relative max-w-md">
          ...
          <input
            ...
            placeholder="搜名称 / 登记号 / 著作权人..."
            value={keyword}
            onChange={(e) => setKeyword(e.target.value)}
          />
        </div>
        <div className="mt-2 text-[11px] text-slate-500">共 {total} 条软著记录</div>
      </div>
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-500 text-sm">暂无匹配的软著记录</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-slate-300">
              <thead className="bg-slate-950/80 text-slate-400 uppercase text-[11px] font-semibold border-b border-slate-800">
                <tr>
                  <th className="px-4 py-4">软著</th>
                  <th className="px-4 py-4">登记号</th>
                  <th className="px-4 py-4">著作权人</th>
                ...
                {items.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-800/30 transition-colors">
                    <td className="px-4 py-4">
                      <div className="font-semibold text-white max-w-[220px] truncate">{c.name}</div>
                      <div className="text-[10px] text-slate-500 mt-0.5">
                        {c.shortName || "无简称"} · {c.sources.length > 0 ? `${c.sources.length} 个资源渠道` : "暂无资源渠道"}
                      </div>
                    </td>
                    <td className="px-4 py-4 text-xs text-slate-300 font-mono">{c.registrationNo || "—"}</td>
                    <td className="px-4 py-4 text-xs text-slate-300">{c.copyrightOwner || "—"}</td>
                    <td className="px-4 py-4">
                      <span
                      ...
                      {c.minSalePrice != null ? (
                        <div className="space-y-0.5">
                          <div className="text-emerald-400">
                            售价 <span className="text-emerald-300 font-medium">{formatPrice(c.minSalePrice, c.currency)}</span>
                          </div>
                          <div className="text-slate-400">
                            成本{" "}
                            {c.minCostPrice != null ? (
                              <span className="text-slate-200 font-medium">{formatPrice(c.minCostPrice, c.currency)}</span>
                            ) : (
                    ...
                    <td className="px-4 py-4 text-right space-x-2 whitespace-nowrap">
                      {user?.role === "SUPER_ADMIN" && c.status === "PENDING" && (
                        <>
                          <button
                            onClick={() => handleAudit(c.id, c.name, true)}
                          ...
                          <button
                            onClick={() => handleAudit(c.id, c.name, false)}
                      ...
                      {(user?.role === "SUPER_ADMIN" || c.createdById === user?.id) && (
                        <Link
                          href={`/admin/software-copyrights/${c.id}/edit`}
