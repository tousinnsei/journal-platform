...
export default function CreateCopyrightPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/v1/admin/software-copyrights", {
        method: "POST",
      ...
      if (!res.ok) throw new Error(data.message || "创建软著失败");

      setSuccessMsg("软著创建成功！正在跳转至列表...");
      setTimeout(() => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <h1 className="text-xl font-bold text-white">新增软著录入表单</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className={section}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="例: 企业知识管理平台软件" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>软件简称</label>
              <input type="text" value={shortName} onChange={(e) => setShortName(e.target.value)} placeholder="例: 知识管理平台" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>登记号</label>
              <input type="text" value={registrationNo} onChange={(e) => setRegistrationNo(e.target.value)} placeholder="例: 2026SR0000001" className={`${inputCls} font-mono`} />
            </div>
            <div>
              <label className={labelCls}>著作权人</label>
              <input type="text" value={copyrightOwner} onChange={(e) => setCopyrightOwner(e.target.value)} placeholder="例: 某某科技有限公司" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>办理类型</label>
              <select value={type} onChange={(e) => setType(e.target.value as "NORMAL" | "URGENT")} className={inputCls}>
              ...
              <div key={s.key} className="border border-slate-800 rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <input type="number" min="0" step="1" required value={s.salePrice} onChange={(e) => updateSource(s.key, { salePrice: e.target.value })} placeholder="例: 2800" className={inputCls} />
                  </div>
                  <div>
        ...
        <div className="flex items-center justify-between bg-slate-900/60 border border-slate-800 rounded-2xl p-6">
          <span className="text-xs text-slate-500">提交后将按所选状态保存软著。</span>
          <button
            type="submit"
            disabled={loading}
