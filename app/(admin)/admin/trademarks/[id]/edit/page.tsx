...
export default function EditTrademarkPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/v1/admin/trademarks/${id}`)
      .then((r) => r.json())
      .then((data) => {
        if (cancelled) return;
        const t = (data as { data?: TrademarkDetail }).data;
        if (t) {
          setName(t.name || "");
          setDescription(t.description || "");
          setNote(t.note || "");
          setStatus(t.status || "DRAFT");
          setOwnerId(t.createdById || null);
          setSources(
            (t.sources || []).map((s) => ({
              id: s.id,
  ...
  const addSource = () => {
    setSources((prev) => [
      ...prev,
      { sourceName: "", contactName: "", times: "1", costPrice: "", salePrice: "", currency: "CNY", status: "ACTIVE", isNew: true },
    ]);
  };
  ...
  const handleSourceSubmit = async (row: SourceRow) => {
    ...
    try {
      const body = JSON.stringify({
        ...
        times: row.times !== "" && !isNaN(Number(row.times)) ? Number(row.times) : 1,
      ...
      const res = await fetch(
        row.isNew ? `/api/v1/admin/trademarks/${id}/sources` : `/api/v1/admin/trademarks/${id}/sources/${row.id}`,
  ...
  const handleSourceDelete = async (row: SourceRow) => {
    ...
    setSourceSaving(true);
    try {
      const res = await fetch(`/api/v1/admin/trademarks/${id}/sources/${row.id}`, { method: "DELETE" });
      const data = await res.json();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "更新商标服务失败");
      setSuccessMsg("商标服务信息更新成功！");
      setTimeout(() => {
  ...
  if (!user || !canEdit) {
    return (
      <div className="p-12 text-center text-slate-400 text-sm space-y-3">
        ...
        <p>仅商标服务创建者本人或超级管理员可编辑该服务</p>
        <button
          onClick={() => router.push("/admin/trademarks")}
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <h1 className="text-xl font-bold text-white">编辑商标服务详情</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className={section}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className={labelCls}>商标服务名称 *</label>
            ...
            <div className="md:col-span-2">
              <label className={labelCls}>服务简介</label>
        ...
        <div className="flex items-center justify-between bg-slate-900/60 border border-slate-800 rounded-2xl p-6">
          <span className="text-xs text-slate-500">保存商标服务基本信息。</span>
          <button
            type="submit"
            disabled={submitting}
          ...
          >
            ...
            <span>{submitting ? "正在保存..." : "保存服务更新"}</span>
          </button>
        </div>
      </form>
          ...
          {sources.map((row, idx) => (
            <div key={row.id ?? `new-${idx}`} className="border border-slate-800 rounded-xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                <div>
                  <label className={labelCls}>对接次数（次）</label>
                  <input type="number" min="0" step="1" value={row.times} onChange={(e) => updateSource(row, { times: e.target.value })} className={inputCls} />
                </div>
                <div>
