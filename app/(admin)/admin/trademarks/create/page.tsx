...
export default function CreateTrademarkPage() {
  ...
  const [sources, setSources] = useState<SourceRow[]>([
    { key: 0, sourceName: "", contactName: "", times: "1", costPrice: "", salePrice: "", currency: "CNY", status: "ACTIVE" },
  ]);
  ...
  const addSource = () => {
    setSources((prev) => [
      ...prev,
      { key: Date.now() + Math.random(), sourceName: "", contactName: "", times: "1", costPrice: "", salePrice: "", currency: "CNY", status: "ACTIVE" },
    ]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        sources: sources
          ...
          .map((s) => ({
            ...
            times: s.times !== "" && !isNaN(Number(s.times)) ? Number(s.times) : 1,
      ...
      const res = await fetch("/api/v1/admin/trademarks", {
        method: "POST",
      ...
      if (!res.ok) throw new Error(data.message || "创建商标服务失败");

      setSuccessMsg("商标服务创建成功！正在跳转至列表...");
      setTimeout(() => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <h1 className="text-xl font-bold text-white">新增商标服务录入表单</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className={section}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="例: 国内商标注册（45 类）" className={inputCls} />
            </div>
            <div>
              ...
              <div key={s.key} className="border border-slate-800 rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <input type="number" min="0" step="1" value={s.times} onChange={(e) => updateSource(s.key, { times: e.target.value })} className={inputCls} />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input type="number" min="0" step="1" required value={s.salePrice} onChange={(e) => updateSource(s.key, { salePrice: e.target.value })} placeholder="例: 1200" className={inputCls} />
                  </div>
                  <div>
        ...
        <div className="flex items-center justify-between bg-slate-900/60 border border-slate-800 rounded-2xl p-6">
          <span className="text-xs text-slate-500">提交后将按所选状态保存商标服务。</span>
          <button
            type="submit"
            disabled={loading}
